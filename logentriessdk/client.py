...
"""
	The **LogentriesSDK.client.LogentriesClient** class provides a Python
	interface to the Logentries API,
	allowing you to programmatically access Logentries resources.

	You can instantiate this class as follows:
		client = LogentriesClient('my_account_key'), where you would
			provide your Logentries
	Account Key as the only parameter, if you don't provide it as a
		parameter, you must set an environment variable called
	LOGENTRIES_ACCOUNT_KEY contaning your account key.

	Once created, you have the following methods at your disposal.  * indicates
...
from logentriessdk.exception import InvalidParametersException
from logentriessdk.exception import InvalidLogSourceException
...
class Client(object):
	...
	def _create_log(self, host, log_name, source, filename=None):
		...
		if success:
			if 'log' in log_data and 'key' in log_data['log']:
				...
				print ('Log information was missing when creating log, log_name=%s, ' +
					'host_name=%s' % (str(log_name), str(host.get_name())))
				return host, None
		else:
			return host, None
	...
	def create_log_http(self, host, log_name, **optionals):
		...
		Required Parameters:
			host (The host object returned by create_host you wish
				to create the log in)
			log_name (The name you would like to give the new log)
		Optional Parameters:
			account_key (Logentries Account Key, can be environment variable
				or passed to method)
	...
	def create_log_agent(self, host, log_name, filename, **optionals):
		"""
		Creates a log of source type 'agent'. To be used in conjunction with
		logentries agent.
		Required Parameters:
			host (The host object returned by create_host you wish to create the log in)
			log_name (The name you would like to give the new log)
	...
	def create_log_syslog(self, host, log_name, **optionals):
		...
		return self._create_log(host, log_name, constants.LOG_SYSLOG)
	...
	def remove_log(self, host, log, **optionals):
		...
		Optional Parameters:
			account_key (Logentries Account Key, can be environment variable or
				passed to method)
	...
	def get_host(self, hostkey, name=None):
		""" Retrieves the host with the specified logkey. If logkey is None,
			then this method returns the first encountered host with name 'name'.
	...
	def get_log(self, logkey, hostkey=None, logname=None):
		"""
		Retrieves the log with the specified logkey. If logkey is None,
			then this method returns the first encountered log with name
			'logname' that is part of host with key 'hostkey'.
		Returns None if no log with key 'logkey' or name 'logname'
			(within host with key 'hostkey') could be retrieved.
		"""
		account = self.get_account()
		if account is None:
			return None
	...
	def get_event(self, eventname, _id=None):
		"""
		Retrieves Event with id '_id' if it is specified or the first event
			whose name matches eventname from a Logentries account. _id has
			precedence over name. So if _id is specified, then eventname is discarded.
		...
		Returns None if no event with name eventname exist or if it could not be
			retrieved.
		"""
		key = '_id'
		if _id is None:
	...
	def create_event(self, eventname, color):
		"""
		Returns the id of the event with name eventname and color 'color' for a
		Logentries account. If an event with name eventname already exist for the
		logentries account, then it is returned and no new event is created (In
		particular, color is discarded in this case). If no event with name eventname
		already exists then one is created and returned.
		Returns None if no event could be created or if its id could not be
			retrieved.
		"""
	...
	def remove_event(self, _id):
		...
		Returns True if and only if event with id _id has been removed
		(whether such an event exists or not). False is returned if the
			attempt for removal failed.
		"""
		request = {
			'request': 'rm_tag',
			'user_key': self.get_account_key(),
	...
	def get_tags(self, logkey):
		...
		Returns a list of tags associated with log with key 'logkeys'
			where each tag is a json structure with keys:
		{
		'object',
		'name',
		'key',
	...
	def get_tag(self, host, logname, tagname, _id=None):
		"""
		Retrieves the first tag encountered with name 'tagname' from the log
			with name logname in host. The tag is a json structure with keys:
		{
		'object',
		'name',
		'key',
		...
		Returns None if no tag could be retrieved with name tagname, for
			log with name logname in host.
		"""
	...
	def create_tag(self, host, logname, tagname, eventid, pattern=''):
		"""
		creates a tag with name 'tagname' for the log with name logname in host.
		The tag is associated event with id 'eventid' and to pattern pattern.
		name a json structure with keys:
		{
		'object',
		'name',
		'key',
		...
		Returns None if no tag could be retrieved with name tagname,
			for log with name logname in host.
		"""
	...
	def remove_tag(self, tagname, pattern, eventid, host, logname):
		"""
		Removes the tag encountered with name 'tagname' from the log with
			name logname in host. The tag is a json structure with keys:
		{
		'object',
		'name',
		'key',
		...
		Returns the deleted tag or None if no tag could be retrieved with
			name tagname, for log with name logname in host.
		"""
		log = client.get_log(host, logname)
		if log is None:
			return None
		tag_data = self.get_tag(host, logname, tagname)
	...
	def get_v2_items(self, item):
		"""
		Retrieves all the items for a Logentries log, using v2 api.
		Returns a list of items where each item is a json structure.
		Returns None if no item could be retrieved.
		"""
		request = {
			'request': 'list',
			'account': self.get_account_key(),
			'acl': self.get_account_key()
		}

		response, success = self._conn.request(request, item)
